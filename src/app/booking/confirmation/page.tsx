        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center">
              <Link href="/tenant" className="text-xl font-bold text-foreground">
                RentZambia
              </Link>
            </div>
          </div>
          ...
          <div className="bg-card rounded-lg shadow-md p-6 mb-8">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between border-b pb-2">
                ...
                <span className="font-medium">{mockBooking.startDate} to {mockBooking.endDate}</span>
              </div>
              
              <div className="flex justify-between border-b pb-2">
                <span className="text-muted-foreground">Amount Paid</span>
                <span className="font-medium">K{mockBooking.totalPrice.toLocaleString()}</span>
              </div>
              
              <div className="flex justify-between border-b pb-2">
                <span className="text-muted-foreground">Booking Status</span>
                <span className="font-medium text-success capitalize">{mockBooking.status}</span>
              </div>
              
              <div className="flex justify-between pt-2">
                <span className="text-muted-foreground">Receipt Number</span>
                <span className="font-medium">RZ-{mockBooking.id}-{Date.now().toString().slice(-4)}</span>
              </div>
            </div>
          </div>

          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <ReceiptGenerator booking={mockBooking} onDownload={handleDownloadReceipt} />
            <Button>
